...
7-1. `targetChannels` 밖의 채널은 성공 조건에 포함하지 않는다. `cardsEnabled=false`면 Instagram 원고·manifest·PNG를 생성하거나 게시하지 않는다.
...
- Instagram 발행과 신규 카드 생성은 비활성이다. 기존 렌더러·템플릿·과거 자산은 삭제하지 않는다.
