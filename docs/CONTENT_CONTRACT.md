...
- 카드를 사용하지 않는 콘텐츠는 `cards: []`로 저장합니다. 기존 카드형 콘텐츠는 8장 규격을 그대로 유지합니다.
...
`npm run build` 성공, 출처 접근 가능성, 금지 표현, 날짜 일치, 중복 slug, 승인 상태와 Telegram 승인 버전을 모두 통과한 커밋만 배포 대상으로 삼습니다. 2026-09-05 이후 마켓브리핑은 `caelus-h.com` 사이트에만 게재하고 카드를 생성하지 않습니다. Instagram 발행과 신규 카드 생성은 비활성화하되 기존 템플릿·렌더러·과거 자산은 재개를 위해 보존합니다. 네이버·티스토리에는 별도의 카드 없는 한국증시 분석글만 발행합니다. 자동화 토큰과 비밀값은 이 저장소에 저장하지 않습니다.
